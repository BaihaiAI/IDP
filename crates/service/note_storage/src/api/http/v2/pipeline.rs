...
pub async fn task_result(
    ...
    let task_file_path = match get_pipeline_output_path(
        team_id,
        project_id,
        &browser_path,
        job_id_u64,
    ...
    // .map_err(|err| IdpGlobalError::NoteError(err))?;
...
async fn nbconvert(
    _nbconvert_bin_path: &str,
    convert_type: &str,
    notebook_path: &str,
) -> Result<(), std::io::Error> {
    let mut cmd = tokio::process::Command::new("jupyter-nbconvert");
    cmd.arg("--to").arg(convert_type).arg(notebook_path);
