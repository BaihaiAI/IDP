...
use crate::handler::workspace::workspace_kubernetes;
...
pub async fn new_file(
    ...
    if path.ends_with(".ipynb") || path.ends_with(".idpnb") {
        let notebook = common_model::entity::notebook::Notebook::new(path);
...
pub async fn model_upload(Json(payload): Json<ModelUploadReq>) -> Result<Rsp<()>, IdpGlobalError> {
    tracing::info!("access workspace model_upload api");
