...
export const CodeCell = (props) => {
  ...
  const quickfixCallback = (cm, content, pos, callback) => {
    ...
    if (Array.isArray(pos)) {
      ...
      if (arrContainMsg) {
        const message = pos[findIndex].data.message;
        const packageName = message.slice(message.indexOf('"') + 1, message.lastIndexOf('"'));
        quickfixSearchPackage(quickfixGetPackageName(packageName), callback)
        return
      }
    } else if (!Array.isArray(pos) && (pos.data.code === 'reportMissingModuleSource' || pos.data.code === 'reportMissingImports')) {
      const message = pos.data.message;
      const packageName = message.slice(message.indexOf('"') + 1, message.lastIndexOf('"'));
      quickfixSearchPackage(quickfixGetPackageName(packageName), callback)
      return;
    }
