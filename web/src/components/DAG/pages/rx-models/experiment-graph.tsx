...
class ExperimentGraph extends GraphCore<BaseNode, BaseEdge> {
  ...
  constructor(expId: string, experimentInstanceId?: string) {
    super({
      history: true,
      frozen: true,
      selecting: {
      ...
      connecting: {
        ...
        validateConnection({
          ...
          // const sourcePord = sourceView.attr.cell.id
          const sourcePord = sourceMagnet.getAttribute('port')!
          ...
          edges.map((edge)=>{
            ...
            if(data?.inputPortId && portId === data.inputPortId && data?.outputPortId === sourcePord){
              hasPort =  true
            }
          });
  ...
  async initialize() {
    ...
    const { experimentId, experimentInstanceId } = this
    this.loading$.next(true)
    try {
      if (experimentInstanceId) {
        await this.loadExperimentInstance(experimentInstanceId)
        await this.loadExecutionStatus(experimentInstanceId)
      } else {
  ...
  async loadExperiment(experimentId: string) {
    try {
      const res = await queryExperiment(experimentId);
      this.experiment$.next(res.experiment as any);
      this.experimentGraph$.next(res.graph as any)
      return { success: true }
    } catch (e) {
      console.error('加载工作流错误', e)
      return { success: false } as any
    }
  }

  // 获取实验状态
  async loadExperimentInstance(experimentInstanceId: string) {
    try {
      const res = await queryExperimentInstance(experimentInstanceId);
      this.experiment$.next(res.experiment as any);
      this.experimentGraph$.next(res.graph as any)
      return { success: true }
    } catch (e) {
      console.error('加载工作流实例错误', e)
      return { success: false } as any
    }
  }
  ...
  async updateExperimentGraph(
    ...
    const newGraph = produce(oldGraph, (nextGraph: any) => {
      if (nodes.length) {
        nextGraph.nodes = [...oldGraph.nodes];
        for (const node of nodes) {
          let isContain = false;
          for (let i = 0; i < nextGraph.nodes.length; i++) {
            if (node.id === nextGraph.nodes[i].id) {
              isContain = true
              nextGraph.nodes[i] = { ...nextGraph.nodes[i], ...node }
              break;
            }
          }
          if (!isContain) {
  ...
  // 保存图元
  saveExperimentGraph = (callback: Function) => {
    const experiment = this.experiment$.getValue();
    const graph = this.experimentGraph$.getValue();
    saveExperiment(experiment, graph).then((res) => {
      const jobId = res.data.jobId;
      if (jobId) {
        this.experiment$.next({ ...experiment, id: jobId });
        this.experimentId = jobId
        callback(true);
      } else {
        callback(false);
      }
  ...
  // 同步保存图元
  saveExperimentGraphSync = async () => {
    const experiment = this.experiment$.getValue();
    const graph = this.experimentGraph$.getValue();
    await saveExperiment(experiment, graph).then((res) => {
      const jobId = res.data.jobId;
      if (jobId) {
        this.experiment$.next({ ...experiment, id: jobId });
        this.experimentId = jobId
  ...
  // 获取执行状态
  loadExecutionStatus = async (experimentInstanceId: string) => {
    this.executionStatusQuerySub?.unsubscribe()
    // 每三秒查询一次执行状态
    this.executionStatusQuerySub = timer(0, 5000).subscribe(
      async (resPromise) => {
        const execStatusRes = await queryGraphStatus(experimentInstanceId)
    ...
    if (type === 'node') {
      const node = this.graph!.addNode(
        new X6DemoNode({
          ...nodeMeta,
          shape: 'ais-rect-port',
          component: <NodeElement experimentId={experimentId} experimentInstanceId={experimentInstanceId} />,
        }),
      ) as BaseNode
  ...
  runGraph = async () => {
    try {
      ...
      // 运行之前先保存
      experimentId = await this.saveExperimentGraphSync();
      const experimentInstanceId = await runGraph(experimentId)
      if (!experimentInstanceId) {
        console.error('执行失败')
      ...
      this.running$.next(true)
      this.clearContextMenuInfo()
      this.loadExecutionStatus(experimentInstanceId) // 发起执行状态查询
      return { success: true }
    } catch (e) {
  ...
  stopRunGraph = async () => {
    try {
      const { experimentInstanceId } = this
      const stopRes = await stopGraphRun(experimentInstanceId)
      this.running$.next(false)
      this.clearContextMenuInfo()
      this.loadExecutionStatus(experimentInstanceId) // 发起执行状态查询
      return stopRes
  ...
  // 修改节点信息
  updateNodeProps = async (nodeInstanceId: string, propName: string, propValue: any) => {
    const cell = this.getCellById(nodeInstanceId)
    const data: object = cell!.getData()
    let newData = { ...data };
  ...
  // 修改节点脚本内容
  updateNodeScript = async (nodeInstanceId: string, script: string) => {
  ...
  // 修改节点执行条件
  updateNodePreNodeRelation = async (nodeInstanceId: string, preNodeRelation: string) => {
    this.updateNodeProps(nodeInstanceId, 'preNodeRelation', preNodeRelation);
  }
  // 修改节点执行优先级
  updateNodePriority = async (nodeInstanceId: string, priority: number) => {
    this.updateNodeProps(nodeInstanceId, 'priority', priority);
  }

  // 修改所有节点执行优先级
  updateAllNodePriority = async (priority: number) => {
...
export const useExperimentGraph = (experimentId: number | string, experimentInstanceId?: string) => {
  const expId = experimentInstanceId ? experimentInstanceId : experimentId.toString()
  let existedExperimentGraph = gModelMap.get(expId)
  if (!existedExperimentGraph) {
    existedExperimentGraph = new ExperimentGraph(experimentId.toString(), experimentInstanceId)
    gModelMap.set(expId, existedExperimentGraph)
  }
  return existedExperimentGraph
}

export const useUnmountExperimentGraph = (experimentId: string, experimentInstanceId?: string) => {
  useEffect(() => {
    return () => {
      const expId = experimentInstanceId ? experimentInstanceId : experimentId
      const existedExperimentGraph = gModelMap.get(expId)
      if (existedExperimentGraph) {
        existedExperimentGraph.dispose()
        gModelMap.delete(expId)
      }
    }
