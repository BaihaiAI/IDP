...
;(function (mod) {
  ...
  function getCodeMirrorClassName(kind) {
    let className
    /*    const Text = 1
    const Keyword = 14
    const Snippet = 15
    ...
    if (
      ...
      className = "CodeMirror-icon-varibale"
    } else if (kind === 2 || kind === 3 || kind === 4) {
      className = "CodeMirror-icon-function"
    } else if (kind === 7) {
      className = "CodeMirror-icon-class"
    } else {
      className = "CodeMirror-icon-keyword"
    }
    return className
  }
  ...
  function Widget(completion, data) {
    ...
    var kindValueList = data.kindValueList
    ...
    for (var i = 0; i < completions.length; ++i) {
      ...
      // className = className + ' CodeMirror-icon-function'
      if(Array.isArray(kindValueList) ){
        className = className + " " + getCodeMirrorClassName(kindValueList[i])
      }
      ...
      if (cur.render) cur.render(elt, data, cur)
        ...
        elt.innerHTML = cur.startsWith(keyword)?cur.replace(keyword,`<span style="color: #2883CB">${keyword}</span>`) :cur
      }else{
    ...
    if(Array.isArray(this.data.originDataList) && this.data.originDataList.length>0){
      const dataItem = this.data.originDataList[0]
      if(typeof dataItem.data==='object'){
        // todo 修改
        const info = {
          label:dataItem.label,
          kind:dataItem.kind,
          workspacePath:dataItem.data.workspacePath,
          filePath:dataItem.data.filePath,
          position:dataItem.data.position,
          symbolLabel:dataItem.data.symbolLabel,
        }
        cm.options.getKeywordItem(info,0)
      }
  ...
  Widget.prototype = {
    ...
    changeActive: function (i, avoidWrap) {
      ...
      if(Array.isArray(this.data.originDataList) && this.data.originDataList.length>0){
        const dataItem = this.data.originDataList[i]
        if(typeof dataItem.data==='object'){
          const info = {
            label:dataItem.label,
            kind:dataItem.kind,
          ...
          cm.options.getKeywordItem(info,i)
        }
      }
